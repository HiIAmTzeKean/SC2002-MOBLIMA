...
public class CustomerBook {
	public static void bookMenu() {
		...
		enum bookMenuState {
			SELECTCINEPLEX, SELECTMOVIE, DISPLAYSHOWTIMES, SELECTSHOWTIME, SELECTSEAT, CUSTOMERDETAILS, DISPLAYPRICE,
			CONFIRMBOOK, END
		};
		...
		while (!complete) {
			switch (state) {
				case SELECTCINEPLEX:
					...
					try{
						String cineplexString = scan.nextLine();
						if(cineplexString.compareTo("0")==0){
							return;
						}
						selectedCineplexID = cs.chooseCineplexfromString(cineplexString);
						if(selectedCineplexID == -1){
							selectedCineplexID = 0;
				...
				case CONFIRMBOOK:
					...
					try {
						System.out.println(
								"Enter 1 to confirm booking, 0 to change selected seat, or -1 to moviegoer main menu");
